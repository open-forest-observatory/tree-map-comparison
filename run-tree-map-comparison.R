## Compare a predicted tree map against an observed tree map. Can be run either interactively via RStudio or from the command line.
# If run from the command line, the call is in the format: Rscript --vanilla /ofo-share/utils/tree-map-comparison/run-tree-map-comparison.R {observed tree points file path} {predicted tree points directory path} {perimeter of observed tree map file path} {temp directory path for intermediate files} {output directory path for eval stats}
# The observed tree map must have heights (in meters) in an attribute `Height` and its status as understory or overstory in the attribute `under_neighbor` (boolean). It must be a geospatial file (e.g. gpkg, geojson) with point geometry.
# The predicted tree map must have heights in the attribute `height` and it must be a .gpkg file with point geometry
...
MAX_PREDICTED_TREES = 50000 # Maximum number of predicted trees, beyond which consider it an extremely poor tree detection and skip it


## Specify input-output files
# If run interactively, specify the input/output file/folder paths here in script
if(interactive()) {
  
  ## Function definitions
  source(here("lib/prep-tree-maps.R"))
  source(here("lib/match-trees.R"))
  source(here("lib/tree-det-stats.R"))
  source(here("lib/connect-matches.R"))
  source(here("lib/area-based-stats.R"))
  source(here("lib/combine-stats-make-corr-plots.R"))
  
  # This 'data-dir.txt' file should contain one line that gives the path to the root of the data directory, which all the files in this block (below) are found in
  ...
  #    This attribution is performed by the script utils/ground_stem_map_assign_under_neighbor.R
  observed_trees_filepath = file.path(data_dir, "ground-truth-stem-map/rectified/ept_trees_01_rectified.geojson")
  ...
  plot_bound_filepath =  file.path(data_dir, "study-area-perimeter/ground_map_mask_precise.geojson")
  ...
  tmp_dir = file.path(data_dir, "temp")
  output_dir = file.path(data_dir, "meta200/itd-evals/itd-eval-fullrun02v3")
  
  # Location of the predicted tree maps to evaluate
  predicted_trees_path = file.path(data_dir, "meta200/drone/L3/ttops_fullrun02/")
  
} else {
  
  # If run from command line, get values from command line arguments
  args = commandArgs(trailingOnly=FALSE)
  # Split on spaces
  #args = str_split(args, fixed(" "))
  which_file = str_which(args, fixed("--file="))[1]

  filearg = args[which_file]|> str_split(fixed("="))
  self_path = filearg[[1]][2]
  ...
  # Function definitions
  source("lib/prep-tree-maps.R")
  source("lib/match-trees.R")
  source("lib/tree-det-stats.R")
  source("lib/connect-matches.R")
  source("lib/area-based-stats.R")
  source("lib/combine-stats-make-corr-plots.R")
  

  # Path to the observed (reference) stem map. It is assumed that this stem map includes exhaustive survey of trees with heights down to 50% of the minimum height class evaluated (currently hard-coded at 10 m, so heights down to at least 5 m). If the dataset has smaller trees, removing them first will make this run faster.
  #    If you wish to include matching for "overstory trees only", this stem map should be pre-attributed with a column called "under_neighbor" that indicates whether a tree is understory or overstory.
  #    This attribution is performed by the script scripts/ground_stem_map_assign_under_neighbor.R
  observed_trees_filepath = args[which_file +1 + 1]
  # Location (folder) of the predicted tree maps to evaluate
  predicted_trees_path = args[which_file + 1 + 2]
  # Path to the field plot boundary. This defines the outer edge of the field (observed trees) plot. It is assumed that the predicted tree stem map extends at least to this boundary if not beyond.
  plot_bound_filepath =  args[which_file + 1 + 3]
  # Location of temp directory (holds intermediate files between the comparison steps) and the directory for comparison outputs
  tmp_dir = args[which_file + 1 + 4]
  output_dir = args[which_file + 1 + 5]
}
